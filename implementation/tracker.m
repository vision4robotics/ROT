...
reg_win = construct_regwindow(params, use_sz, small_filter_sz);
target_region = center_region_extraction(use_sz, small_filter_sz);
...
while true
    ...
    % Do training
    wf = train_ROT(params, model_xf, yf, reg_win, target_region);
    ...
    if params.visualization
        ...
        if seq.frame == 1  %first frame, create GUI
            ...
            sampled_scores_display = fftshift(response);
            figure(fig_handle);
            imagesc(im_to_show);
            hold on;
            resp_handle = imagesc(xs, ys, sampled_scores_display); colormap jet;
            alpha(resp_handle, 0.3);
            rectangle('Position',rect_position_vis, 'EdgeColor','g', 'LineWidth',2);
            text(10, 10, int2str(seq.frame), 'color', [0 1 1]);
            hold off;
            %             axis off;axis image;set(gca, 'Units', 'normalized', 'Position', [0 0 1 1])
